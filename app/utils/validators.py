...
class Validator:
        ...
        for field, field_rules in rules.items():
            ...
            for rule in field_rules:
                try:
                    if not rule(value):
                        self.errors.append(
                            f"Campo '{field}' no cumple la regla: {rule.__name__}"
                        )
                        break
    ...
    def raise_if_errors(self, custom_message: str = None):  # type:ignore
    ...
    def validate_ci(
        self, ci: str, expedicion: str = None  # type:ignore
    ) -> bool:
        """
        ...
        if len(ci_str) < self.CI_MIN_LENGTH or len(ci_str) > self.CI_MAX_LENGTH:
            self.errors.append(
                f"El CI debe tener entre {self.CI_MIN_LENGTH} y {self.CI_MAX_LENGTH} caracteres"
            )
            return False
    ...
    def validate_nombre_completo(self, nombres: str, apellidos: str) -> bool:
        ...
        if len(nombres.strip()) > self.NOMBRE_MAX_LENGTH:
            self.errors.append(
                f"Los nombres no pueden exceder {self.NOMBRE_MAX_LENGTH} caracteres"
            )
            return False
        ...
        if len(apellidos.strip()) > self.NOMBRE_MAX_LENGTH:
            self.errors.append(
                f"Los apellidos no pueden exceder {self.NOMBRE_MAX_LENGTH} caracteres"
            )
            return False
    ...
    def validate_fecha_nacimiento(
        self, fecha_nacimiento: Union[str, date], edad_minima: int = 16
    ) -> bool:
        """
    ...
    def validate_email(self, email: str, requerido: bool = False) -> bool:
        ...
        if len(email) > self.EMAIL_MAX_LENGTH:
            self.errors.append(
                f"El email no puede exceder {self.EMAIL_MAX_LENGTH} caracteres"
            )
            return False
    ...
    def validate_telefono(
        self, telefono: str, pais: str = "BO", requerido: bool = False
    ) -> bool:
        """
        ...
        if (
            len(telefono) < self.TELEFONO_MIN_LENGTH
            or len(telefono) > self.TELEFONO_MAX_LENGTH
        ):
            self.errors.append(
                f"El teléfono debe tener entre {self.TELEFONO_MIN_LENGTH} y {self.TELEFONO_MAX_LENGTH} dígitos"
            )
            return False
            ...
            if pais == "BO":
                ...
                if number_type not in tipos_validos:
                    self.warnings.append(
                        "El número podría no ser móvil ni fijo boliviano"
                    )
    ...
    def validate_cupos(self, cupos_totales: int, cupos_disponibles: int = None) -> bool:
        ...
        if cupos_disponibles is not None:
            ...
            if cupos_disponibles > cupos_totales:
                self.errors.append(
                    "Los cupos disponibles no pueden ser mayores al total"
                )
                return False
    ...
    def validate_monto(
        self, monto: Any, minimo: Decimal = None, maximo: Decimal = None
    ) -> bool:
        """
    ...
    def validate_descuento(
        self, descuento: Any, maximo: Decimal = Decimal("100")
    ) -> bool:
        """
    ...
    def validate_fecha_promocion(
        self, fecha_limite: date, fecha_inicio: date = None
    ) -> bool:
        """
        ...
        fecha_limite_date = (
            ...
            else datetime.strptime(fecha_limite, "%Y-%m-%d").date()
        )
    ...
    def validate_matricula_data(self, data: Dict[str, Any]) -> bool:
        ...
        if data["modalidad_pago"] not in modalidades_validas:
            self.errors.append(
                f"Modalidad de pago inválida. Válidas: {', '.join(modalidades_validas)}"
            )
        ...
        if "estado_pago" in data and data["estado_pago"]:
            ...
            if data["estado_pago"] not in estados_pago_validos:
                self.errors.append(
                    f"Estado de pago inválido. Válidos: {', '.join(estados_pago_validos)}"
                )

        if "estado_academico" in data and data["estado_academico"]:
            estados_academicos_validos = [
                "PREINSCRITO",
                "INSCRITO",
                "EN_CURSO",
                "CONCLUIDO",
            ...
            if data["estado_academico"] not in estados_academicos_validos:
                self.errors.append(
                    f"Estado académico inválido. Válidos: {', '.join(estados_academicos_validos)}"
                )
        ...
        if all(
            field in data
            for field in ["monto_total", "descuento_aplicado", "monto_final"]
        ):
            try:
                ...
                if total - descuento != final:
                    self.errors.append(
                        "El monto final no coincide con (total - descuento)"
                    )
    ...
    def validate_pago_data(self, data: Dict[str, Any]) -> bool:
        ...
        if data["forma_pago"] not in formas_pago_validas:
            self.errors.append(
                f"Forma de pago inválida. Válidas: {', '.join(formas_pago_validas)}"
            )
    ...
    def sanitize_string(
        self, texto: str, max_length: int = None, allow_html: bool = False
    ) -> str:
        """

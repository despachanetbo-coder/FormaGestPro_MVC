...
class MovimientoCajaController:
    ...
    def registrar_movimiento(self, datos: Dict[str, Any]) -> Tuple[bool, str, Optional[MovimientoCajaModel]]:
        """
        ...
        try:
            ...
            if not self._current_usuario:
                return False, "No hay usuario autenticado para registrar movimiento", None
            ...
            if movimiento_id:
                ...
                logger.info(f"✅ Movimiento de caja registrado: {movimiento.tipo} - Bs. {movimiento.monto:.2f}")
    ...
    def eliminar_movimiento(self, movimiento_id: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            if self._current_usuario and self._current_usuario.rol not in ['ADMIN', 'CONTADOR']:
            ...
            if movimiento.referencia_tipo and movimiento.referencia_id:
                return False, f"No se puede eliminar movimiento con referencia {movimiento.referencia_tipo} #{movimiento.referencia_id}"
            
            # Eliminar movimiento
            if MovimientoCajaModel.delete(movimiento_id):
                logger.info(f"Movimiento {movimiento_id} eliminado por usuario {self._current_usuario.id if self._current_usuario else 'desconocido'}")
    ...
    def anular_movimiento(self, movimiento_id: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            datos_anulacion = {
                ...
                'usuario_id': self._current_usuario.id if self._current_usuario else None,
                'forma_pago': movimiento.forma_pago if hasattr(movimiento, 'forma_pago') else 'EFECTIVO'
            }
            
            exito, mensaje, movimiento_anulacion = self.registrar_movimiento(datos_anulacion)
            if exito:
                # Marcar el movimiento original como anulado
                movimiento.anulado = True
                movimiento.save()
                
                logger.info(f"Movimiento {movimiento_id} anulado por movimiento {movimiento_anulacion.id}")
    ...
    def registrar_ingreso_pago(self, pago_id: int) -> Tuple[bool, str, Optional[MovimientoCajaModel]]:
        """
        ...
        try:
            ...
            if self._existe_movimiento_para_referencia('PAGO', pago_id):
                return False, f"Ya existe un movimiento registrado para el pago {pago_id}", None
    ...
    def registrar_ingreso_generico(self, ingreso_id: int) -> Tuple[bool, str, Optional[MovimientoCajaModel]]:
        """
        ...
        try:
            ...
            if not ingreso:
                return False, f"No se encontró ingreso genérico con ID {ingreso_id}", None
            ...
            if self._existe_movimiento_para_referencia('INGRESO_GENERICO', ingreso_id):
                return False, f"Ya existe un movimiento registrado para el ingreso {ingreso_id}", None
            ...
            logger.error(f"Error al registrar ingreso por ingreso genérico {ingreso_id}: {e}")
            return False, f"Error interno: {str(e)}", None
    
    def registrar_egreso_gasto(self, gasto_id: int) -> Tuple[bool, str, Optional[MovimientoCajaModel]]: 
        """
        ...
        try:
            ...
            if self._existe_movimiento_para_referencia('GASTO', gasto_id):
                return False, f"Ya existe un movimiento registrado para el gasto {gasto_id}", None
    ...
    def registrar_ingreso_factura(self, factura_id: int) -> Tuple[bool, str, Optional[MovimientoCajaModel]]:
        """
        ...
        try:
            ...
            if self._existe_movimiento_para_referencia('FACTURA', factura_id):
                return False, f"Ya existe un movimiento registrado para la factura {factura_id}", None
    ...
    def obtener_movimientos_rango(
        ...
        try:
            ...
            logger.error(f"Error al obtener movimientos en rango {fecha_inicio} - {fecha_fin}: {e}")
            return []
    ...
    def obtener_movimientos_por_referencia(
        ...
        try:
            ...
            logger.error(f"Error al obtener movimientos por referencia {referencia_tipo}: {e}")
            return []
    
    def obtener_resumen_diario(self, fecha: Optional[Union[str, date]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            movimientos = self.obtener_movimientos_rango(f"{fecha_str} 00:00:00", f"{fecha_str} 23:59:59")
    ...
    def obtener_resumen_mensual(self, año: int = None, mes: int = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            dias_con_movimientos = len([d for d in por_dia.values() if d['ingresos'] > 0 or d['egresos'] > 0])
            promedio_ingresos_diario = total_ingresos / dias_con_movimientos if dias_con_movimientos > 0 else 0
            promedio_egresos_diario = total_egresos / dias_con_movimientos if dias_con_movimientos > 0 else 0
    ...
    def obtener_estadisticas(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ingresos_hoy = sum(float(m.monto) for m in movimientos_hoy if m.tipo == 'INGRESO')
            egresos_hoy = sum(float(m.monto) for m in movimientos_hoy if m.tipo == 'EGRESO')
        ...
        try:
            ...
            if movimientos:
                ...
                try:
                    ...
                    fecha_anterior = (fecha_ini_obj - timedelta(days=1)).strftime('%Y-%m-%d')
            ...
            if formato.lower() == 'html':
                return self._generar_reporte_html(
                    movimientos, fecha_inicio, fecha_fin,
                    total_ingresos, total_egresos, saldo_periodo,
                    saldo_inicial, saldo_final, incluir_detalle
                )
            else:
    ...
    def _generar_reporte_texto(
        ...
        if incluir_detalle and movimientos:
            ...
            for i, mov in enumerate(movimientos, 1):
                ...
                reporte.append(f"{i:3d}. {fecha_str} {tipo_simbolo} Bs. {float(mov.monto):,.2f}")
                reporte.append(f"     {mov.descripcion[:70]}")
                
                # Información adicional
                info_extra = []
                if mov.referencia_tipo:
                    info_extra.append(f"Ref: {mov.referencia_tipo} #{mov.referencia_id}")
    ...
    def _generar_reporte_html(
        ...
        if incluir_detalle:
            ...
            if movimientos:
                ...
                for i, mov in enumerate(movimientos, 1):
                    clase_fila = "ingreso-row" if mov.tipo == 'INGRESO' else "egreso-row"
                    clase_monto = "monto-positivo" if mov.tipo == 'INGRESO' else "monto-negativo"
    ...
    def _validar_datos_movimiento(
        ...
        if 'referencia_tipo' in datos and datos['referencia_tipo']:
            ...
            tipos_validos = ['PAGO', 'GASTO', 'FACTURA', 'INGRESO_GENERICO', 'ANULACION', 'OTRO']
            if ref_tipo not in tipos_validos:
                errores.append(f"Tipo de referencia inválido. Válidos: {', '.join(tipos_validos)}")
        
        # Validar referencia_id si se proporciona referencia_tipo
        if 'referencia_tipo' in datos and datos['referencia_tipo'] and datos['referencia_tipo'] != 'OTRO':
            if 'referencia_id' not in datos or not datos['referencia_id']:
                errores.append(f"ID de referencia requerido para tipo '{datos['referencia_tipo']}'")
    ...
    def _existe_movimiento_para_referencia(self, referencia_tipo: str, referencia_id: int) -> bool:
        """
        ...
        try:
            ...
            logger.error(f"Error al verificar movimiento para {referencia_tipo}/{referencia_id}: {e}")
            return False
            ...
            if hasattr(pago, 'matricula_id') and pago.matricula_id:
                try:
                    ...
                    if matricula:
                        ...
                        if estudiante:
                            descripcion += f"estudiante {estudiante.nombre} {estudiante.apellido}"
    ...
    def realizar_cierre_caja(self, fecha: Optional[Union[str, date]] = None) -> Tuple[bool, str, Dict[str, Any]]:
        """
        ...
        try:
            ...
            if results and results[0]['count'] > 0:
                return False, f"Ya existe un cierre de caja para la fecha {fecha_str}", resumen
            ...
            if exito:
                ...
                resumen['usuario_cierre'] = self._current_usuario.nombre if self._current_usuario else 'Sistema'
                
                logger.info(f"Cierre de caja realizado para {fecha_str} por usuario {self._current_usuario.id if self._current_usuario else 'desconocido'}")
                return True, f"Cierre de caja realizado exitosamente para {fecha_str}", resumen
            else:

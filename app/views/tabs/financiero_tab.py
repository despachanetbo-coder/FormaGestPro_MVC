...
class FinancieroTab(BaseTab):
    ...
    def setup_ui(self):
        ...
        self.combo_filtro.addItems(
            ["Todos", "Pendientes", "Completados", "Atrasados", "Cancelados"]
        )
    ...
    def mostrar_pagos_en_tabla(self, pagos):
        ...
        try:
            ...
            for fila, pago in enumerate(pagos):
                ...
                # Código
                codigo = (
                    getattr(pago, "codigo_pago", "")
                    or getattr(pago, "codigo", "")
                    or ""
                )
                ...
                # Fecha
                fecha = (
                    getattr(pago, "fecha_pago", "") or getattr(pago, "fecha", "") or ""
                )
                ...
                # Método de pago
                metodo = (
                    getattr(pago, "metodo_pago", "")
                    or getattr(pago, "metodo", "")
                    or ""
                )
    ...
    def actualizar_paginacion(self):
        ...
        try:
            ...
            self.total_pages = max(
                1, (total_pagos + self.records_per_page - 1) // self.records_per_page
            )
            ...
            if total_pagos > 0:
                self.lbl_contador.setText(
                    f"Mostrando {start_idx + 1}-{end_idx} de {total_pagos} registros"
                )
            else:
    ...
    def agregar_nuevo_pago(self):
        ...
        try:
            ...
            btn_guardar.clicked.connect(
                lambda: self.guardar_nuevo_pago(
                    txt_codigo.text(),
            ...
            QMessageBox.critical(
                self, "Error", f"No se pudo abrir el formulario: {str(e)}"
            )

    def guardar_nuevo_pago(
        self, codigo, fecha, estudiante, concepto, monto, metodo, estado, dialog
    ):
        """Guardar nuevo pago en la base de datos"""
        try:
            if not codigo or not estudiante:
                QMessageBox.warning(
                    self, "Advertencia", "Código y Estudiante son obligatorios"
                )
                return
            ...
            nuevo_pago = IngresoModel.create(**pago_data)
    ...
    def ver_detalles_pago(self, pago):
        ...
        try:
            ...
            dialog.setWindowTitle(
                f"Detalles del Pago - {getattr(pago, 'codigo_pago', '')}"
            )
            dialog.setModal(True)
            dialog.resize(500, 400)
            ...
            QMessageBox.warning(
                self, "Error", f"No se pudieron mostrar los detalles: {str(e)}"
            )
    ...
    def marcar_como_pagado(self, pago):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"No se pudo actualizar el pago: {str(e)}"
            )
    ...
    def eliminar_pago(self, pago):
        ...
        try:
            ...
            if respuesta == QMessageBox.Yes:
                ...
                if resultado:
                    QMessageBox.information(
                        self, "Éxito", "Pago eliminado correctamente"
                    )
            ...
            QMessageBox.critical(
                self, "Error", f"No se pudo eliminar el pago: {str(e)}"
            )
    ...
    def generar_reporte(self):
        ...
        try:
            ...
            # Conectar botones
            radio_mensual.clicked.connect(
                lambda: self.generar_reporte_tipo("mensual", dialog)
            )
            radio_trimestral.clicked.connect(
                lambda: self.generar_reporte_tipo("trimestral", dialog)
            )
            radio_anual.clicked.connect(
                lambda: self.generar_reporte_tipo("anual", dialog)
            )
            radio_pendientes.clicked.connect(
                lambda: self.generar_reporte_tipo("pendientes", dialog)
            )
            ...
            QMessageBox.critical(
                self, "Error", f"No se pudo generar el reporte: {str(e)}"
            )
    ...
    def generar_reporte_tipo(self, tipo, dialog):
        ...
        try:
            ...
            completados = sum(
                1
                for p in self.pagos_data
                if getattr(p, "estado", "").lower() in ["completado", "pagado"]
            )
            pendientes = sum(
                1
                for p in self.pagos_data
                if getattr(p, "estado", "").lower() == "pendiente"
            )
            total_ingresos = sum(
                float(getattr(p, "monto", 0) or 0)
                for p in self.pagos_data
                if getattr(p, "estado", "").lower() in ["completado", "pagado"]
            )
            ...
            for pago in self.pagos_data:
                metodo = (
                    getattr(pago, "metodo_pago", "No especificado") or "No especificado"
                )

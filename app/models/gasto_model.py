...
"""
Modelo para Gastos Operativos usando PostgreSQL directamente.
"""
import logging
...
class GastoModel(BaseModel):
    ...
    def __init__(self, **kwargs):
        ...
        self.docente_id = kwargs.get(
            "docente_id"
        )  # <-- IMPORTANTE: Relación con docente
    ...
    def _validar(self):
        ...
        if self.subcategoria and self.categoria in self.SUBCATEGORIAS:
            if self.subcategoria not in self.SUBCATEGORIAS[self.categoria]:
                raise ValueError(
                    f"Subcategoría inválida para {self.categoria}. Válidas: {self.SUBCATEGORIAS[self.categoria]}"
                )
    ...
    def save(self):
        ...
        try:
            ...
            MovimientoCajaModel.registrar_egreso_gasto(
                gasto_id=self.id, monto=self.monto, descripcion=descripcion
            )
            logger.info(
                f"💰 Movimiento de caja (EGRESO) registrado automáticamente para gasto {self.id}"
            )

        except Exception as e:
            logger.error(
                f"Error al registrar movimiento de caja para gasto {self.id}: {e}"
            )
    ...
    def buscar_por_categoria(cls, categoria):
        ...
        query = (
            f"SELECT * FROM {cls.TABLE_NAME} WHERE categoria = ? ORDER BY fecha DESC"
        )
